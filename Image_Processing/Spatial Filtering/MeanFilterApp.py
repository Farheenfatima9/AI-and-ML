...
def filtering(img, fil):
    ...
    for i in range(a, rows - a):
        for j in range(a, cols - a):
            ...
            z = np.sum(region * fil)
            z = np.clip(z, 0, 255)  # prevent overflow
            output[i - a, j - a] = z
...
mask = creating_mask()
if mask is not None:
    image = cv2.imread(r"D:\Semester 6\DIP\Labs\Lab 06\camera.png", 0)
    cv2.imshow("Original Image", image)
    cv2.waitKey()

    padding_size = mask.shape[0] // 2
    padded_image = padding(image, padding_size)

    filtering(padded_image, mask)
